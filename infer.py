...
parser.add_argument('--infer_input_file', type=str, nargs='+', default=[" "])
...
parser.add_argument("--max_reframing_length", type=int, default=50)
...
dataloader_kwargs = {
    ...
    'max_reframing':args.max_reframing_length
}
...
str_bos = torch.LongTensor([toker.convert_tokens_to_ids("<s>")])
...
for infer_idx, infer_input_file in enumerate(args.infer_input_file):
    ...
    infer_dataloader = inputter.infer_dataloader(
        ...
        toker,
        reframing_data_df,
        reframing_situation_and_thought_emb,
        sentence_model,
        classifier,
        **dataloader_kwargs
    )
    ...
    for batch, posts, references,reframing_thoughts, sample_ids in infer_dataloader:
        batch = {k: v.to(device) if isinstance(v, Tensor) else v for k, v in batch.items()}
        batch.update(generation_kwargs)
        batch.update({"str_bos":str_bos,"supportor_id":supportor_id,"seeker_id":seeker_id})
        ...
        for idx in range(len(sample_ids)):
            p = posts[idx]
            r = references[idx]
            rt = reframing_thoughts[idx]
            rt= toker.decode(rt)
            # 写入真实和预测的策略
            s = batch_other_res['cls_strat_id'][0]
            pre_s = encoded_info['pred_strat_id'][0]
            s = toker.decode(s)
            pre_s = toker.decode(pre_s)
            if not args.only_encode:
                ...
                tmp_res_to_append = {'strategy':s,'pre_strategy':pre_s,'sample_id': sample_ids[idx],'post': p, 'response': r, 'persona': tmp_persona,'generation': g,"reframing_thought":rt}
                #print('> context:            ', p)
                #print('> generation:', g)
            else:
